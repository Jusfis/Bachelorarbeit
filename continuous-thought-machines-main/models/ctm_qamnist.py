...
class ContinuousThoughtMachineQAMNIST_KAN(ContinuousThoughtMachineKAN):
        def __init__(self,
                     iterations,
                     d_model,
                     d_input,
        ...
        def get_kv_for_step(self, total_iterations_for_digits, total_iterations_for_question, stepi, x, z,
                            prev_input=None, prev_kv=None):
            """Get the key-value for the current step."""
            is_digit_step, is_question_step, is_answer_step = self.determine_step_type(total_iterations_for_digits,
            ...
            if is_digit_step:
                ...
                if prev_input is not None and torch.equal(current_input, prev_input):
                    return prev_kv, prev_input
                is_index_step, is_operator_step = self.determine_index_operator_step_type(total_iterations_for_digits,
                                                                                          stepi)
                if is_index_step:
                    kv = self.index_backbone(current_input)
                elif is_operator_step:
        ...
        def forward(self, x, z, track=False):
            ...
            self.decay_params_action.data = torch.clamp(self.decay_params_action, 0, 15)  # Fix from github user: kuviki
            self.decay_params_out.data = torch.clamp(self.decay_params_out, 0, 15)
            r_action, r_out = torch.exp(-self.decay_params_action).unsqueeze(0).repeat(B, 1), torch.exp(
                -self.decay_params_out).unsqueeze(0).repeat(B, 1)
            ...
            for stepi in range(total_iterations):
                ...
                kv, prev_input = self.get_kv_for_step(total_iterations_for_digits, total_iterations_for_question, stepi,
                                                      x, z, prev_input, prev_kv)
                prev_kv = kv
                ...
                # --- Calculate Synchronisation for Output Predictions ---
                synchronization_out, decay_alpha_out, decay_beta_out = self.compute_synchronisation(activated_state,
                                                                                                    decay_alpha_out,
                                                                                                    decay_beta_out,
                                                                                                    r_out,
                                                                                                    synch_type='out')
            ...
            # --- Return Values ---
            if track:
                return predictions, certainties, synchronization_out, np.array(pre_activations_tracking), np.array(
                    post_activations_tracking), np.array(attention_tracking), np.array(embedding_tracking)
            return predictions, certainties, synchronization_out