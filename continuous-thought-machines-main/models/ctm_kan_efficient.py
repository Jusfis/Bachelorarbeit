...
from models.modules import ParityBackbone, SynapseUNET, Squeeze, SuperLinear, LearnableFourierPositionalEncoding, \
...
#todo postactivation production implementation
        ...
        for synch_type, size in (('action', self.synch_representation_size_action),
    ...
    def get_d_backbone(self):
        ...
        if self.backbone_type == 'shallow-wide':
            ...
            if '18' in self.backbone_type or '34' in self.backbone_type:
                if self.backbone_type.split('-')[1] == '1':
                    return 64
                elif self.backbone_type.split('-')[1] == '2':
                    return 128
                elif self.backbone_type.split('-')[1] == '3':
                    return 256
    ...
    def forward(self, x, track=False):
        ...
        # --- Return Values ---
        if track:
            return predictions, certainties, (np.array(synch_out_tracking), np.array(synch_action_tracking)), np.array(
