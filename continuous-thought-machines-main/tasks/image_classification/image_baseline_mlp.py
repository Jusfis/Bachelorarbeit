...
from models.baseline_mlp import SimpleMLP
...
def parse_args():
    ...
    # Model Selection
    parser.add_argument('--model', type=str, default='ctm', choices=['ctm', 'lstm', 'ff','resnet','mlp'], help='Model type to train.')
    ...
    parser.add_argument('--strict_reload', action=argparse.BooleanOptionalAction, default=True,
                        help='Should use strict reload for model weights.')  # Added back
...
def imagenet_baseline_model(args, config, run):
    ...
    if args.model == 'ctm':
        ...
        if args.dataset in ['cifar10', 'cifar100']:
            flat_input_dim = 3 * 32 * 32  # 3072
        elif args.dataset == 'imagenet':
            flat_input_dim = 3 * 224 * 224  # 150528
        print("Initialising MLP with flat input dimension:", flat_input_dim)
        model = SimpleMLP(
        ...
        for bi in range(start_iter, args.training_iterations):
            ...
            with torch.autocast(device_type="cuda" if "cuda" in device else "cpu", dtype=torch.float16,
                ...
                if args.model == 'ctm':
                    ...
                    loss, where_most_certain = image_classification_loss(predictions, certainties, targets,
                                                                         use_most_certain=True)
                    accuracy = (predictions.argmax(1)[torch.arange(predictions.size(0),
                ...
                elif args.model in ['ff', 'resnet', 'mlp']:
            ...
            if (bi % args.track_every == 0 or bi == args.warmup_steps) and (bi != 0 or args.reload_model_only):
                ...
                with torch.no_grad():  # Should use inference_mode? CTM/LSTM scripts used no_grad
                    ...
                    with tqdm(total=len(loader), initial=0, leave=False, position=1, dynamic_ncols=True) as pbar_inner:
                        for inferi, (inputs, targets) in enumerate(loader):
                            ...
                            if args.model == 'ctm':
                                ...
                                loss, where_most_certain = image_classification_loss(these_predictions, certainties,
                                                                                     targets, use_most_certain=True)
                                all_predictions_list.append(
                                    these_predictions.argmax(1).detach().cpu().numpy())  # Shape (B, T)
                                all_predictions_most_certain_list.append(these_predictions.argmax(1)[
                    ...
                    if args.model in ['ctm', 'lstm']:
                        ...
                        current_train_accuracies = np.mean(all_predictions == all_targets[..., np.newaxis],
                                                           axis=0)  # Mean over batch dim -> Shape (T,)
                ...
                if args.model in ['ctm', 'lstm']:
                    try:  # For safety
                        ...
                        predictions_viz, certainties_viz, _, pre_activations_viz, post_activations_viz, attention_tracking_viz = model(
                        ...
                        img_to_gif = np.moveaxis(np.clip(
                            inputs_viz[imgi].detach().cpu().numpy() * np.array(dataset_std).reshape(len(dataset_std), 1,
                                                                                                    1) + np.array(
                                dataset_mean).reshape(len(dataset_mean), 1, 1), 0, 1), 0, -1)
...
def run_sweep():
    ...
    with wandb.init(entity="justus-fischer-ludwig-maximilian-university-of-munich", project="baseline-imagenet") as run:
        ...
        imagenet_baseline_model(args, config, run)
...
if __name__ == "__main__":
    ...
    if args.useWandb == 1:
        ...
        sweep_configuration = {
            "program": "image_baseline_mlp.py",
            "name": "baseline-imagenet",
            "method": "random",
            "metric": {
                "name": "Train/Losses",
                "goal": "minimize"
        ...
        sweep_id = wandb.sweep(sweep_configuration, project="baseline-imagenet")
        wandb.agent(sweep_id, function=run_sweep, count=50)

    else:
        # ------------------- RUN WITHOUT WANDB ------------------------ #
        imagenet_baseline_model(args, None, None)
