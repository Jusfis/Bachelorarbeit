    ...
    try:
        ...
        for i, frame in enumerate(frames):
            ...
            if frame.shape[0] != frame_height or frame.shape[1] != frame_width:
                print(
                    f"Warning: Frame {i} has different dimensions {frame.shape[:2]}! Expected ({frame_height},{frame_width}). Skipping.")
                continue

            current_dims = len(frame.shape)
            if current_dims != expected_dims:
                print(
                    f"Warning: Frame {i} has inconsistent dimensions ({current_dims}D vs expected {expected_dims}D). Skipping.")
                continue
        ...
        print(
            "Please ensure FFmpeg is installed and its directory is included in your system's PATH environment variable.")
...
def plot_neural_dynamics(post_activations_history, N_to_plot, save_location, axis_snap=False, N_per_row=5,
                         which_neurons_mid=None, mid_colours=None, use_most_active_neurons=False):
    ...
    fig_synch, axes_synch = plt.subplot_mosaic(mosaic=mosaic, figsize=(figscale * mosaic.shape[1] * aspect_ratio * 0.2,
                                                                       figscale * mosaic.shape[0] * 0.2))
    fig_mid, axes_mid = plt.subplot_mosaic(mosaic=mosaic, figsize=(figscale * mosaic.shape[1] * aspect_ratio * 0.2,
                                                                   figscale * mosaic.shape[0] * 0.2), dpi=200)
    ...
    random_indices = np.random.choice(np.arange(post_activations_history.shape[-1]), size=N_to_plot,
    ...
    with tqdm(total=N_to_plot, initial=0, leave=False, position=1, dynamic_ncols=True) as pbar_inner:
        ...
        for ndx in range(N_to_plot):
            ...
            for traces_s_here, traces_m_here in zip(traces_s, traces_m):
                ax_s.plot(np.arange(len(traces_s_here)), traces_s_here, linestyle='-', color=c_s, alpha=0.05,
                          linewidth=0.6)
                ax_m.plot(np.arange(len(traces_m_here)), traces_m_here, linestyle='-', color=c_m, alpha=0.05,
                          linewidth=0.6)
            ...
            if axis_snap and np.all(np.isfinite(traces_s[0])):
                ax_s.set_ylim([np.min(traces_s[0]) - np.ptp(traces_s[0]) * 0.05,
                               np.max(traces_s[0]) + np.ptp(traces_s[0]) * 0.05])
                ax_m.set_ylim([np.min(traces_m[0]) - np.ptp(traces_m[0]) * 0.05,
...
def make_classification_gif(image, target, predictions, certainties, post_activations, attention_tracking, class_labels,
                            save_location):
    cmap_viridis = sns.color_palette('viridis', as_cmap=True)
    cmap_spectral = sns.color_palette("Spectral", as_cmap=True)
    figscale = 2
    with tqdm(total=post_activations.shape[0] + 1, initial=0, leave=False, position=1,
        ...
        for stepi in np.arange(0, n_steps, 1):
            ...
            attention_interp = \
            torch.nn.functional.interpolate(torch.from_numpy(attention_now).unsqueeze(0), image.shape[:2],
                                            mode='bilinear')[0]
            attention_interp = (attention_interp.flatten(1) - attention_interp.flatten(1).min(-1, keepdim=True)[0]) / (
                        attention_interp.flatten(1).max(-1, keepdim=True)[0] -
                        attention_interp.flatten(1).min(-1, keepdim=True)[0])
            ...
            axes['certainty'].plot(np.arange(len(certainties_now)), certainties_now, 'k-', linewidth=figscale * 1,
                                   label='1-(normalised entropy)')
            for ii, (x, y) in enumerate(zip(np.arange(len(certainties_now)), certainties_now)):
                is_correct = predictions[:, ii].argmax(-1) == target
                if is_correct:
                    axes['certainty'].axvspan(ii, ii + 1, facecolor='limegreen', edgecolor=None, lw=0, alpha=0.3)
                else:
            ...
            for hi in range(min(8, n_heads)):
                ...
                for i in range(len(these_route_steps) - 1):
                    ...
                    ax_overlay.arrow(x_coords[i], y_coords[i], dx, dy, linewidth=1.6 * arrow_scale * 1.3,
                                     head_width=1.9 * arrow_scale * 1.3, head_length=1.4 * arrow_scale * 1.45,
                                     fc='white', ec='white', length_includes_head=True, alpha=1)
                    ax_overlay.arrow(x_coords[i], y_coords[i], dx, dy, linewidth=1.6 * arrow_scale,
                                     head_width=1.9 * arrow_scale, head_length=1.4 * arrow_scale, fc=route_colours[i],
                                     ec=route_colours[i], length_includes_head=True)
