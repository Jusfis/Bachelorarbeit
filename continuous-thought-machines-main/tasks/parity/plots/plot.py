...
def plot_metrics_seaborn(test_acc_files, test_loss_files, train_acc_files, train_loss_files, version):
    # Daten aggregieren
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, f'parity_10_5_loss_accuracies_{version}.png'), dpi=300)
    plt.show()
...
test_acc_files_mlp = [os.path.join(BASE_DIR, f) for f in
                  ['test_accuracies_mlp_10_5_parity.csv']]
test_loss_files_mlp = [os.path.join(BASE_DIR, f) for f in
                   ['test_losses_mlp_10_5_parity.csv']]
train_acc_files_mlp = [os.path.join(BASE_DIR, f) for f in
                   ['train_accuracies_mlp_10_5_parity.csv', ]]
train_loss_files_mlp = [os.path.join(BASE_DIR, f) for f in
                    ['train_losses_mlp_10_5_parity.csv']]

# Funktion aufrufen
plot_metrics_seaborn(test_acc_files_mlp, test_loss_files_mlp, train_acc_files_mlp, train_loss_files_mlp, 'mlp')

# Listen mit den Dateinamen für die 3 Seeds erstellen
test_acc_files_kan = [os.path.join(BASE_DIR, f) for f in
                  ['test_accuracies_kan_10_5_parity.csv']]
test_loss_files_kan = [os.path.join(BASE_DIR, f) for f in
                   ['test_losses_kan_10_5_parity.csv']]
train_acc_files_kan = [os.path.join(BASE_DIR, f) for f in
                   ['train_accuracies_kan_10_5_parity.csv', ]]
train_loss_files_kan = [os.path.join(BASE_DIR, f) for f in
                    ['train_losses_kan_10_5_parity.csv']]

plot_metrics_seaborn(test_acc_files_kan, test_loss_files_kan, train_acc_files_kan, train_loss_files_kan, 'kan')